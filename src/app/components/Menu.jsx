...
const Menu = () => {
    ...
    const [navColor, setNavColor] = useState('bg-green-200 '); // Estado para el color de fondo de la barra de navegación
    ...
    return (
        <header className={`${navColor} text-white flex items-center justify-between p-4 md:p-2`}>
            <div className='flex items-center'>
                <a href='#inicio1' className='text-black font-roboto text-3xl tracking-wider flex items-center' onClick={() => changeLanguage(i18n.language === 'en' ? 'es' : 'en')}>
                    <VscCode className='mr-100' />
                    {t('code')}
                </a>
                <nav className='hidden md:block ml-8 space-x-4'>
                    <a href='#' className='hover:bg-indigo-800 rounded-full px-10 py-10 text-xl'>{t('home')}</a>
